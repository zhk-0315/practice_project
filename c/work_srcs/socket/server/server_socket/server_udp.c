...
int init_server_udp(void)
{
    ...
    if (CHECK_FD(g_srv_udpfd)) {
        return 0;
    }

    g_srv_udpfd = socket(AF_INET, SOCK_DGRAM, 0);
    if (!CHECK_FD(g_srv_udpfd)) {
        lc_err_logout("socket g_srv_udpfd error");
        return -1;
    }

    iret = setsockopt(g_srv_udpfd, SOL_SOCKET, SO_REUSEADDR, &optval_, sizeof(optval_));
    if (iret != 0) {
        lc_err_logout("setsockopt g_srv_udpfd error");
    }

    lcaddr.sin_family = AF_INET;
    lcaddr.sin_addr.s_addr = INADDR_ANY;
    lcaddr.sin_port = htons(g_server_udp_port);
    iret = bind(g_srv_udpfd, (struct sockaddr*)&lcaddr, g_addrlen);
    if (iret != 0) {
        lc_err_logout("bind g_srv_udpfd error");
        return -1;
    }

    add_fd_to_srv_epoll(g_srv_udpfd, EPOLLIN | EPOLLET);
...
int destroy_server_udp(void)
{
    ...
    del_fd_from_srv_epoll(g_srv_udpfd);

    return close(g_srv_udpfd);
}

_Bool epoll_trigge_udpfd(int fd)
{
    return (fd == g_srv_udpfd);
}
...
int send_msg_by_udp(lc_msg_package_t* msg_pack, struct sockaddr_in* _lcaddr)
{
    ...
    TXsize = sendto(g_srv_udpfd, msg_pack, sizeof(lc_msg_package_t), 0,
...
int send_udp_cli_msg(socket_handle_t* _socket, lc_msg_package_t* _msgbuf)
{
    ...
    TXsize = sendto(g_srv_udpfd, _msgbuf, sizeof(lc_msg_package_t), 0,
